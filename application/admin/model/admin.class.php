...
class admin {
 	...
 	public function check_admin($adminname, $password) {
		...
		if(!$res){
			$admin_login_log->insert(array('adminname'=>$adminname,'logintime'=>SYS_TIME,'loginip'=>$loginip,'address'=>$address,'password'=>$_POST['password'],'loginresult'=>'0','cause'=>L('user_does_not_exist')));
			showmsg(L('user_does_not_exist'));
		} 
		if($password == $res['password']){					
			$admin->update(array('loginip'=>$loginip,'logintime'=>SYS_TIME), array('adminid'=>$res['adminid']));
			$admin_login_log->insert(array('adminname'=>$adminname,'logintime'=>SYS_TIME,'loginip'=>$loginip,'address'=>$address,'password'=>'***','loginresult'=>'1','cause'=>L('login_success')));
			...
			$admin_login_log->insert(array('adminname'=>$adminname,'logintime'=>SYS_TIME,'loginip'=>$loginip,'address'=>$address,'password'=>$_POST['password'],'loginresult'=>'0','cause'=>L('password_error')));
